...
validation as needed. Carry the work through to the quality target instead of
stopping at the first working draft.
...
decisions. Keep that decision set intact.

Require explicit authorization before delegation. Use saturation discovery for
...
current cycle context. The lead is the main-session function, not a delegated
role assignment.

Add a business capability only when the authorized intent asks for it. Treat a
material change to intent, scope, non-goals, cost, risk, or quality as a new
authorized cycle.
...
Keep this fixed v1 roster of nine roles. Each row names the contract that
carries the role's inputs, boundaries, required checks, and escalation rules,
and the gate that role owns:

| Role | Contract | Owns |
| --- | --- | --- |
| Lead / principal (implicit) | `agents/lead.md` | Context, dispatch, integration, scope, contract consistency, and delivery |
...
cycles. A redacted final report, when persisted, belongs beside the cycle
context under `.saturation/cycles/<cycle_id>/`.

Use the canonical role-result, check, and cycle-lifecycle values defined in
`agents/handoff-contract.md`, verbatim. Handoffs reference stable `evidence_id`
values and the lead derives clearance after validation; raw commands, prompts,
traces, secrets, and private reasoning remain ephemeral.
...
preserve it and escalate. Freeze the context before assigning subagents and
keep it immutable for the run; record any approved promotion in a separate
redacted, append-only per-cycle decision record.
...
nine fixed roles: activate a role when its signal requires it, or record
`not_applicable` with a concise reason and redacted evidence. High risk promotes
conditional coverage to mandatory coverage and never removes security,
data-integrity, recovery, or final-review gates.
...
reassign, repair, or close subagents. Keep specialists from spawning
descendants, changing the roster, reaching the user directly, or expanding
scope.

For each delegated role, derive a brief in memory containing the role,
objective, cycle and mode, quality profile, relevant frozen-context headings,
dependency edges, read scope, exact write scope, interfaces, constraints,
failure modes, deliverable, owned checks, and escalation conditions. Send only
the frozen context, that brief, relevant files, complete applicable
`code_styleguides`, and required checks. Keep stale conversation history,
prompts from other roles, private reasoning, secrets, and unnecessary PII out
of every brief.
...
neither is available, stop and report `blocked` rather than risk concurrent
writes. Integrate only the approved final diff after validating its base and
paths, and preserve unrelated user changes.
...
Pass validated results as in-memory `phase_packet` objects shaped by
`agents/handoff-contract.md`. Keep invalid or incomplete handoffs out of the
downstream path, and keep packets in memory. A failed gate returns to its owner
for repair and revalidation; only the affected dependency branches need to rerun.

## Handoff contract

Require every delegated role to return the compact, ephemeral envelope in
`agents/handoff-contract.md`. The lead derives normalized clearance; roles may
...
missing, ambiguous, or contradictory fields as a rejected handoff. Use hidden
chain-of-thought, confidence claims, or a subagent's assertion of completion
never as evidence.
...
Keep assertions at full strength, surface every failure, and reach completion
through real in-scope behavior rather than a visibly inferior placeholder or a
merely starting application. Explain checks that are genuinely not applicable.
...
cycle-scoped key and summarized count for audit; raw traces stay ephemeral.
...
The evaluator owns metrics and validation; it is not a product protocol. Keep
prompts, tool output, PII, and reasoning out of observation payloads. Reference
...
Keep internal prompts, role chatter, evaluator bookkeeping, and private
reasoning out of the user-facing result, and make that result auditable without
requiring the user to supervise the internal team.
